...
class WalkRead(Env):
    ...
    def __init__(self):
        ...
        # 1.8 degrees: 0.0314 rad --> 2 m/s
        # 1.1 degrees: 0.0192 rad --> 1 m/s
        self._pitch_amp_upper_bound = 0.0314
        self._pitch_amp_lower_bound = 0.0192
        self._yaw_amp_upper_bound = 0.0628
        self._yaw_amp_lower_bound = 0.0384

        self._pitch_freq_upper_bound = 2.25
        self._pitch_freq_lower_bound = 1.7
        self._yaw_freq_upper_bound = 1.125
        self._yaw_freq_lower_bound = .85

        self._walking_speed_upper_bound = 2
        self._walking_speed_lower_bound = 1
        ...
        # Define the action space
        self.action_space = Box(low=-1, high=1, shape=(3,))   # 2 dof eyeball rotation and walking speed
    ...
    def calc_pitch_amp(self, speed):
        slope =  self._pitch_amp_upper_bound - self._pitch_amp_lower_bound
        y_int = self._pitch_amp_lower_bound - slope
        return slope*speed + y_int

    def calc_pitch_freq(self, speed):
        slope = self._pitch_freq_upper_bound - self._pitch_freq_lower_bound
        y_int = self._pitch_freq_lower_bound - slope
        return slope * speed + y_int
    ...
    def step(self, action):
        # dd
        action[2] = self.normalise(action[2], -1, 1, self._walking_speed_lower_bound, self._walking_speed_upper_bound)
        self._walking_speed = action[2]

        # Apply the locomotion to the agent - non-stationary environment
        self._data.ctrl[self._agent_y_motor_mjidx] = np.clip(
            self._data.ctrl[self._agent_y_motor_mjidx]+self._walking_speed/20, *self._agent_y_translation_range
        )
        ...
        pitch_amp = self.calc_pitch_amp(self._walking_speed)
        yaw_amp = pitch_amp * 2
        pitch_freq = self.calc_pitch_freq(self._walking_speed)
        yaw_freq = pitch_freq/2

        pitch_period = self._action_sample_freq / pitch_freq
        yaw_period = self._action_sample_freq / yaw_freq

        pitch = pitch_amp * np.sin(2 * np.pi * self._steps / pitch_period)
        yaw = yaw_amp * np.sin(2 * np.pi * self._steps / yaw_period)
